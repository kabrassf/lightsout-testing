...
(function ($) {
  ...
  * Core Logger >>
  *
  * Create colorful console logs for use with all plugins.
  * - Writes to a log file
  * - Used for simple messages and simple debugging.
  *
  ------------------------------------------------------------------- */
  function Logger() {
    ...
    var _logs = [];
    var _silent = options.silent || false;
    // const _forceConsole = options.force || false
    var _pluginName = options.pluginName || 'Logger';
    var _date = new Date();
    var _timestamp = `${_date.getHours()}:${_date.getMinutes()}`;
    var _logFormat = [{ name: 'System', accent: '#092496', color: '#0b0b14', text: '#6bb1e6', padding: '5px 0' }, { name: 'Normal', accent: '#60e816', color: '#0b0b14', text: '#6bb1e6', padding: '5px 0' }, { name: 'Debug', accent: '#f4e242', color: '#0b0b14', text: '#6bb1e6', padding: '5px 0' }, { name: 'Error', accent: '#e81717', color: '#0b0b14', text: '#6bb1e6', padding: '5px 0' }];

    var _generateMessage = function _generateMessage() {
      var level = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
      var message = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';

      var format = _logFormat[level];
      if (navigator.userAgent.toLowerCase().indexOf('chrome') > -1) {
        return [`\n %c %c %c [${_pluginName}] => ${message} %c \n\n`, `background: ${format.accent}; padding: ${format.padding};`, `padding: ${format.padding};`, `color: ${format.text}; background:${format.color}; padding:${format.padding};`, `background: ${format.accent}; padding: ${format.padding};`];
      } else {
        return `[${_pluginName}] => ${message}`;
      }
    };

    var _generateLog = function _generateLog() {
      var level = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
      var message = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';

      var levelName = _logFormat[level].name;
      _logs.push(`[${_timestamp}] [${levelName}] => ${message}`);
    };

    return {
      log(level, message, object) {
        _logs.push(_generateLog(level, message));
        if (_silent) {
          return;
        }
        if (object) {
          window.console.groupCollapsed(`[${_pluginName}] => ${message}`);
          window.console.log(object);
          window.console.groupEnd();
        } else {
          window.console.log.apply(console, _generateMessage(level, message));
        }
      },
      ...
      groupLog(name) {
        ...
        var logs = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];
        ...
        logs.forEach(function (log) {
          if (!groupStart) {
            switch (collapsed) {
              case true:
                window.console.groupCollapsed(`[${_pluginName}] ${name}`);
                break;
              case false:
                window.console.group(`[${_pluginName}] ${name}`);
                break;
            }
            groupStart = true;
          }
          _this.log(log.level, log.message, log.object);
        });
        window.console.groupEnd();
      },

      createLogFile() {
        FileIO.tryWriteFile(FileIO.projectPath(), _logs, 'utf8', function () {
      ...
      storedLog() {
        return _logs.filter(function (log) {
          return typeof log !== 'undefined';
        });
      }
    };
  }
  /* Create instance of a logger for use across core plugin.*/
  $.CoreLogger = Logger({
    pluginName: 'LTN-Core'
  });
  /** Print important version numbers to console.*/
  var versionLogs = [{ level: 0, message: 'Core Version: ' + LTN.VERSION }, { level: 0, message: 'RMMV Version: ' + Utils.RPGMAKER_VERSION }];
  $.CoreLogger.groupLog('Versions', versionLogs);
  /** -----------------------------------------------------------------
   * Utilities >>
...
(function (SM, Params) {
  ...
  SM.resizeToResolution = function () {
    ...
    if (Params.fitToBrowserWindow && !Utils.isNwjs()) {
  ...
  SM.openDebugTools = function () {
    if (Params.autoOpenConsole && Utils.isNwjs() && Utils.isOptionValid('test')) {
      var _debugWindow = require('nw.gui').Window.get().showDevTools();
      _debugWindow.moveTo(0.1, 0.5);
      window.focus();
    }
    if (Params.autoShowFps) {
      Graphics.showFps();
    }
  };
...
Scene_Boot.prototype.start = function () {
  if (LTN.Core.Params.autoSkipTitle) {
